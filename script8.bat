...
echo %code%-%numero%-0000-MMD-GER-Memorial Descritivo
...
for /L %%i in (1, 1, %pav%) do (
    ...
    if !teste! gtr 9 (
        echo %code%-%numero%-00!teste!-PLA-!pavText!-Planta infraestrutura
    ) else (
        echo %code%-%numero%-000!teste!-PLA-!pavText!-Planta infraestrutura
    )
...
for /L %%i in (1, 1, %pav%) do (
    ...
    if !pavCurrent! gtr 0 (
	    ...
	    rem Formata o número da planta com quatro dígitos, preenchendo com zeros à esquerda
    if !teste! gtr 9 (
        echo %code%-%numero%-00!teste!-PLA-!pavText!-Planta distribuição
    ) else (
        echo %code%-%numero%-000!teste!-PLA-!pavText!-Planta distribuição
    )
)


	echo %code%-%numero%-00%currentIndex%-GER-Planta Detalhamento
